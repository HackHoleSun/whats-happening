            ...
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                state.event.date?.let { date ->
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Text(formatDate(date), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                }
...
private val previewEvent = ScrapedEvent(
    id       = "1",
...
private fun EventDetailLoadingPreview() {
    WhatsHappeningTheme {
        Surface { EventDetailSheetContent(state = DetailUiState(event = previewEvent, isLoading = true), onOpenInBrowser = {}) }
    }
}
        ...
        Surface {
            EventDetailSheetContent(
                state = DetailUiState(
                    event = previewEvent,
                    isLoading = false,
                    description = "Pridružite nam se na čarobnoj jazz večeri u srcu Novog Sada! Nastup poznatih lokalnih muzičara uz ukusna pića i prijatnu atmosferu.",
                    imageUrl = null,
                ),
                onOpenInBrowser = {},
            )
